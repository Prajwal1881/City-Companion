...
class PlanCard extends StatelessWidget {
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            // Host row
            Row(children: [
              CircleAvatar(radius: 17, backgroundColor: color, child: Text(
                (plan['host_name'] as String? ?? 'U').substring(0, 1),
                style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w800),
              )),
              const SizedBox(width: 10),
              Expanded(child: RichText(text: TextSpan(children: [
                TextSpan(text: plan['host_name'] ?? '', style: const TextStyle(fontWeight: FontWeight.w800, color: AppColors.ink, fontSize: 13)),
                const TextSpan(text: ' is hosting', style: TextStyle(color: AppColors.sub, fontSize: 13)),
              ]))),
            ...
            Text(plan['title'] ?? '', style: const TextStyle(
              fontWeight: FontWeight.w800, fontSize: 17, color: AppColors.ink, height: 1.25,
            )),
            ...
            // CTA
            _buildCTA(context, isHost: plan['is_host'] == true, hasJoined: plan['has_joined'] == true, pct: pct, color: color),
          ]),
        ),
      ]),
    );
  ...
  Widget _buildCTA(BuildContext context, {required bool isHost, required bool hasJoined, required double pct, required Color color}) {
    if (isHost) {
      return SizedBox(
        ...
        child: ElevatedButton(
          ...
          child: const Text('View Details', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 14, color: Colors.white)),
        ),
      );
    }
    ...
    if (!hasJoined) {
      return SizedBox(
        width: double.infinity,
        child: ElevatedButton(
          onPressed: pct >= 1.0 ? null : () => onJoin(plan['id']?.toString() ?? ''),
          style: ElevatedButton.styleFrom(
      ...
      children: [
        ...
        Expanded(
          child: ElevatedButton(
            ...
            child: const Text('💬 Chat', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 14, color: Colors.white)),
          ),
        ),
      ],
    );
